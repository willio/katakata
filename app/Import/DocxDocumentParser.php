...
final class DocxDocumentParser
{
    ...
    private function detectOriginalCategory(array $paragraphs): ?string
    {
        foreach (array_slice($paragraphs, 0, 8) as $paragraph) {
            if (preg_match('/^(kama\p{L}*)\s+by\s+.+$/iu', $paragraph['text'], $matches) === 1) {
                return $matches[1];
            }
        }
    ...
    /** @return list<array{text:string,markdown:string,style:string,list:bool,quote:bool}> */
    private function paragraphs(string $xml): array
    {
        ...
        foreach ($dom->getElementsByTagNameNS('http://schemas.openxmlformats.org/wordprocessingml/2006/main', 'p') as $paragraph) {
            ...
            [$text, $markdown] = $this->paragraphContent($paragraph);
            if ($text === '') {
                continue;
            }
    ...
    /** @return array{string,string} */
    private function paragraphContent(DOMElement $paragraph): array
    {
        $plain = '';
        $markdown = '';
        foreach ($paragraph->getElementsByTagNameNS('http://schemas.openxmlformats.org/wordprocessingml/2006/main', 'r') as $run) {
            ...
            $markdown .= $this->formatRun($text, $bold, $italic);
        }

        return [$this->cleanText($plain), $this->cleanText($markdown)];
    }
    ...
    private function detectAuthor(array $metadata, array $paragraphs, ?string $fallback): array
    {
        foreach (array_slice($paragraphs, 0, 8, true) as $index => $paragraph) {
            if (preg_match('/^(?:(?:kama\p{L}*\s+)?by|oleh|written by|author)\s*[:\-]?\s*(.+)$/iu', $paragraph['text'], $matches) === 1) {
